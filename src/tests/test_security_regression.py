...
class TestSQLInjectionGridTypeAllowlist:
    ...
    def test_grid_500_accepted(self):
        assert validate_grid_type("grid_500") == "research_grid_500m"
    ...
    def test_table_name_is_alphanumeric_or_underscore(self):
        ...
        for gt in ("voronoi", "square", "research", "grid_500"):
            ...
            assert table.replace("_", "").isalnum(), (
                f"Unsafe character in table name: {table!r}"
            )
...
# ---------------------------------------------------------------------------
# 3. RATE LIMIT - SightingThrottle is configured at 100/hour
# ---------------------------------------------------------------------------
...
class TestRadiusParameterCap(TestCase):
    ...
    def test_huge_radius_capped_not_500(self):
        ...
        assert response.status_code != 500, (
            "Huge radius caused HTTP 500 - DoS vector not fixed"
        )

    def test_radius_cap_applied_in_viewset(self):
        """
        Check the radius cap formula min(radius, 50) in isolation. NOTE: this
        re-implements the formula inline (below) and does NOT call
        get_queryset(), so the viewset's real cap is not exercised here.
        """
        from sightings.views import SightingViewSet
        from rest_framework.test import APIRequestFactory as RF
...
PORTFOLIO_COMPUTE_ENDPOINTS = [
    ...
    # NOTE: two endpoints were intentionally re-gated with IsBearerAuthenticated
    # and are NO LONGER public (anon-rejection asserted in test_crash_regression.py):
    #   - /api/analytics/config/apply-preset/ (crash fix C1 / hardening S2)
    #   - /api/research/runs/clear/ (destructive DELETE of all runs / hardening S2)
