...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING("DRY RUN - zmiany nie zostana zapisane"))

        # Delete all existing configs
        if not dry_run:
            deleted_count, _ = ResearchConfig.objects.all().delete()
            self.stdout.write(self.style.WARNING(f"Usunieto {deleted_count} konfiguracji"))
        else:
        ...
        for preset_id, preset in RESEARCH_PRESETS.items():
            ...
            if not dry_run:
                ResearchConfig.objects.create(
                    ...
                    regime_threshold_urban=config_data.get('regime_threshold_urban', 0.15),
