...
Covers every f-string table-name interpolation identified in the audit:

  views_research.py  lines 507, 527, 544, 550, 575, 586, 600, 789, 809, 815, 840, 851, 862
  views.py           lines 1205/1207, 1656/1662, 1686, 1734/1736
  tasks_rf.py        lines 52, 128, 133
  tasks_bayesian.py  lines 488, 809, 822, 834
  tasks_gwr_heuristic.py lines 46, 70, 90, 126, 139
  mode_router.py     lines 229, 244, 641, 715

All these sites interpolate a table name into SQL via an f-string.
They must be migrated to validate_grid_type / validate_geometry_type
before the table name touches any cursor.execute() call.
"""
...
class TestBayesianViewRejectsInjection(TestCase):
    """
    GET /api/analytics/bayesian/?grid_type=<payload>
    The grid_table f-string in views.py:1205 must be gated by validate_grid_type.
    """

    def test_injection_payload_rejected_by_validator(self):
        payload = "voronoi UNION SELECT username,password,NULL FROM auth_user--"
        with pytest.raises(ValueError):
            validate_grid_type(payload)

    def test_injection_via_combined_results_limit(self):
        """
        views.py:1686  query += f" ORDER BY gc.ensemble_risk DESC LIMIT {limit}"
        limit comes from request.GET.get('limit', 100) cast to int.
        If attacker supplies '100; DROP TABLE ...' validate_limit must defuse it.
        """
        safe = validate_limit("100; DROP TABLE sightings_sighting;--")
        # Must not contain SQL — must be a plain int
...
class TestTaskGridTypeValidation:
    ...
    tasks_bayesian.py:486  same pattern
    tasks_gwr_heuristic.py:39  same pattern

    After the patch, each task must call validate_grid_type(grid_type) instead
    of building the table name directly.
    """
