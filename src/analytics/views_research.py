...
def config_list(request):
    ...
    Public in this portfolio sandbox (AllowAny): configs are demo research
    parameters (active predictors, model settings), not sensitive data.
    """
...
def run_pipeline(request):
    ...
    Runs ASYNCHRONOUSLY via Celery. Returns immediately with run_id.
...
def pipeline_status(request):
    ...
    return Response(
        {
            "active_config": _serialize_config(active_config)
            if active_config
            else None,
            ...
            return Response(
                {
                    ...
                    "n_cells": 0,
                    "available": False,
                    "message": 'Brak danych. Kliknij "Odswiez" aby wygenerowac.',
                }
            )
...
def generate_preview(request):
    ...
    logger.info(
        f"generate_preview: queuing task for geometry_type={geometry_type}, table={target_table}"
    )
...
def spatial_impacts(request):
    ...
    try:
        ...
        logger.warning("spatial_impacts: analytics_spatial_result not initialized")
        return Response(
            {
                "available": False,
