...
def compute_rf(self, grid_type: str = "voronoi", run_id: str = None):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                f"SELECT id, {feature_cols}, sighting_count FROM {grid_table}"
            )
            rows = cursor.fetchall()
        debug.success(
            "load_data",
            f"Loaded {len(rows)} cells",
            values={"count": len(rows), "grid_type": grid_type},
            start_time=t,
        )
        ...
        debug.success(
            "prepare_features",
            f"{len(X)} valid samples",
            values={
                "total_cells": len(rows),
                "valid_cells": len(X),
                "skipped_null": skipped["all_null"],
                "skipped_zero": skipped["all_zero"],
        ...
        if sum(y) == 0:
            debug.warning(
                "train_model",
                "No positive samples!",
                values={"positive": 0, "negative": len(y)},
            )
            ...
            debug.warning(
                ...
                values={"positive": len(y), "negative": 0},
            )
            ...
            feature_importances = dict(
                zip(RF_FEATURES, [round(float(f), 4) for f in rf.feature_importances_])
            )
        ...
        with connection.cursor() as cursor:
            ...
            cursor.executemany(
                f"UPDATE {grid_table} SET rf_score = %s, updated_at = NOW() WHERE id = %s",
                update_data,
            )
        debug.success(
            "save_results",
            f"Updated {len(update_data)} cells",
            values={"updated": len(update_data), "table": grid_table},
            start_time=t,
        )
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                f"SELECT AVG(rf_score), MIN(rf_score), MAX(rf_score), STDDEV(rf_score), COUNT(DISTINCT rf_score), COUNT(*) FROM {grid_table} WHERE rf_score IS NOT NULL"
            )
            stats = cursor.fetchone()
        verification = {
            "rf_mean": round(float(stats[0]), 4) if stats[0] else None,
            "rf_min": round(float(stats[1]), 4) if stats[1] else None,
            "rf_max": round(float(stats[2]), 4) if stats[2] else None,
            "rf_std": round(float(stats[3]), 4) if stats[3] else None,
            "unique_values": stats[4] or 0,
            "total_with_rf": stats[5] or 0,
        }

        if verification["unique_values"] > 10:
            debug.success(
                "verify",
                "Verification passed - good variance",
                values=verification,
                start_time=t,
            )
        else:
