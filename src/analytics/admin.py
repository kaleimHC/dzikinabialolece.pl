...
class PriorConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Identification", {"fields": ("id", "model_version", "is_active")}),
        (
            "Prior Distribution",
            {
...
class BayesianResultAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Cell Info", {"fields": ("id", "execution", "cell_id", "grid_type")}),
        (
            "Posterior Summary",
            {
                "fields": (
    ...
    def convergence_status(self, obj):
        ...
        if obj.r_hat < 1.01 and (obj.ess_bulk is None or obj.ess_bulk > 400):
            return format_html(
                '<span style="color: green;">R-hat={:.3f}</span>', obj.r_hat
            )
        elif obj.r_hat < 1.05:
            return format_html(
                '<span style="color: orange;">R-hat={:.3f}</span>', obj.r_hat
            )
        else:
            return format_html(
                '<span style="color: red;">R-hat={:.3f}</span>', obj.r_hat
            )
...
class ParameterConfigurationAdmin(admin.ModelAdmin):
    ...
    def stationarity_check(self, obj):
        ...
        return format_html(
            '<span style="color: {};">ρ+δ = {}</span>', color, f"{total:.3f}"
        )
    ...
    def weights_display(self, obj):
        ...
        return format_html(
            '<span style="color: {};">Σ = {}</span>', color, f"{total:.3f}"
        )
