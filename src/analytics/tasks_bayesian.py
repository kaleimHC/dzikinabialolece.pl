...
def _run_r_script_docker(
    ...
    logger.info(f"  DEBUG: RUN_ID={env.get('RUN_ID', 'NOT IN ENV')}, GRID_TYPE={env.get('GRID_TYPE', 'NOT IN ENV')}")
    try:
...
def elicit_priors(
    ...
    logger.info(f"  Inputs: H_rel={h_rel}, ARI={ari}, bandwidth={bandwidth}, kappa={kappa}")
    ...
    try:
        ...
        logger.info(f"  length_scale ~ LogNormal({length_scale_meanlog}, {length_scale_sdlog})")
...
def compute_bayesian_ssm(
    ...
    logger.info(f"  grid_type={grid_type}, iterations={mcmc_iterations}, chains={mcmc_chains}")
    ...
    # Update progress
    cache.set(f'bayesian_progress:{run_id}', {
        'running': True,
        'step': 'validation',
        'progress': 0,
        'error': None,
    ...
    try:
        ...
        # R script path
        r_script = '/app/r_scripts/06_bayesian_ssm.R'
        ...
        env['PRIOR_LENGTH_MEANLOG'] = str(prior_data.get('length_scale', {}).get('alpha', 6))
        env['PRIOR_LENGTH_SDLOG'] = str(prior_data.get('length_scale', {}).get('beta', 0.3))

        logger.info(f"  Running R script via Docker: 06_bayesian_ssm.R")
        ...
        r_env = {
            ...
            'PRIOR_LENGTH_MEANLOG': str(prior_data.get('length_scale', {}).get('alpha', 6)),
            'PRIOR_LENGTH_SDLOG': str(prior_data.get('length_scale', {}).get('beta', 0.3)),
        }
...
def invalidate_bayesian_cache(
    ...
    """
    logger.info(f"OP-B05: Invalidate Bayesian cache")
    ...
    try:
        ...
        for pattern in patterns:
            try:
                ...
                deleted = cache.delete_pattern(pattern) if hasattr(cache, 'delete_pattern') else 0
