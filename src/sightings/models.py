...
class Sighting(models.Model):
    ...
    class Meta:
        ...
        # DB-level enum CHECKs (defense-in-depth, mirrors migration 0006):
        # a serializer-bypassing write (raw SQL / admin / mgmt cmd) cannot
        # persist a status/boar_count/sighting_type no button emits.
        constraints = [
            models.CheckConstraint(check=models.Q(status__in=["pending", "verified", "rejected", "duplicate"]), name="sighting_status_valid"),
            models.CheckConstraint(check=models.Q(boar_count__in=["1", "2-3", "4-10", "10+"]), name="sighting_boar_count_valid"),
            models.CheckConstraint(check=models.Q(sighting_type__in=["encounter", "ryjowisko"]), name="sighting_type_valid"),
        ]
...
class GridCell(models.Model):
    ...
    # PUWG 1992 (EPSG:2180) = polski uklad metryczny; spatialWarsaw tesseluje wewnetrznie w EPSG:3857 (eta.R/W_matrix.R), wybor 2180 to decyzja projektu dla obliczen metrycznych
...
# in analytics/views_pub.py and analytics/views_admin.py - the ORM is
# intentionally bypassed for these layers.
