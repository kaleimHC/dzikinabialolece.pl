#!/usr/bin/env Rscript
# 02_spatial_models.R - SAR/SEM według Kopczewskiej
# ŹRÓDŁO: spatialreg (lagsarlm, errorsarlm); workflow wzorowany na spatialWarsaw::BootSpatReg
...
cat("============================================================\n")
cat("02_spatial_models.R - SAR/SEM według Kopczewskiej\n")
cat("============================================================\n")
...
# EPS guards against zero area only. Zero counts are handled by the +1 continuity
# correction (log((count+1)/area)), which keeps empty cells finite (log(1/area))
# instead of collapsing to a log(eps) outlier spike.
EPS_INTENSITY <- 1e-6
if (y_formula == "log_intensity") {
  # DEFAULT: sighting intensity per area - the actual risk signal (reports per km2).
  # +1 continuity: empty cells -> log(1/area), giving a smooth gradient (no -13.8 spike).
  # On the regular 500 m grid area is ~constant so this tracks log(count+1);
  # on Voronoi (count==1) it reduces to -log(area), i.e. local sighting density.
  gridcells_raw$Y <- log((gridcells_raw$sighting_count + 1) /
                           pmax(gridcells_raw$area_km2, EPS_INTENSITY))
} else if (y_formula == "inv_pop") {
  # Population-avoidance PROXY (opt-in, explicit hypothesis - NOT a risk measure).
  gridcells_raw$Y <- 1.0 / pmax(gridcells_raw$population, 1)
} else if (y_formula == "log_pop") {
  # Population-avoidance PROXY (opt-in). Models -log(population), not sightings.
...
if (use_regime && regime_type %in% c("binary", "trinary")) {
  ...
  if (n_regimes > 1) {
    # Regime is DERIVED from forest_cover + building_density (see 04_variable_y.R),
    # so interacting regime with those same predictors is self-collinear (VIF blow-up,
    # uninterpretable interactions). Keep the regime-defining predictors as pooled main
    # effects and interact regime only with the independent predictors.
    # "0 + regime" gives each regime its own intercept.
    regime_defining <- c("forests_z", "buildings_z")
    interact_z <- setdiff(z_names, regime_defining)
    maineff_z  <- intersect(z_names, regime_defining)
    terms <- "0 + regime"
    if (length(interact_z) > 0) {
      terms <- paste(terms, sprintf("regime:(%s)", paste(interact_z, collapse = " + ")),
                     sep = " + ")
    }
    if (length(maineff_z) > 0) {
      terms <- paste(terms, paste(maineff_z, collapse = " + "), sep = " + ")
      cat(sprintf("  Regime: %s kept as pooled main effects (avoid self-collinearity).\n",
                  paste(maineff_z, collapse = ", ")))
    }
...
# Skip impacts for regime-interaction models: a single direct/indirect/total row per
# variable is invalid when effects vary by regime (LeSage-Pace impacts assume pooled
# coefficients). Impacts are reported only for the pooled SAR/SDM.
regime_model_active <- use_regime && exists("n_regimes") && n_regimes > 1

if (best_result$type %in% c("SAR", "SDM") && !regime_model_active) {
...
cat("\n============================================================\n")
cat("02_spatial_models.R ZAKONCZONY\n")
cat("============================================================\n")
cat(sprintf("Y formula: %s\n", y_formula))
cat(sprintf("Model: %s\n", best_result$type))
