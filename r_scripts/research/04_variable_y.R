...
#   inv_pop   — Y = 1 / population (wariancja z mianownika)
#   log_pop   — Y = -log(population) (skosne rozklady)
#   binary    — Y = 1 if count > 0, else 0 (probit/logit)
#
...
VALID_FORMULAS <- c("count_pop", "inv_pop", "log_pop", "log_count", "binary")
...
y_sql <- switch(y_formula,
  ...
  "log_count" = "ln(sighting_count + 1)",  # NEW: Direct measure of boar presence
