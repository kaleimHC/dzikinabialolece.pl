...
#
# Testy:
#   Moran I     - autokorelacja przestrzenna reszt (domyslnie wlaczone)
#   LM tests    - LM-lag vs LM-error (domyslnie wlaczone)
...
# Validate TARGET_TABLE against allowlist - consistency with the sibling scripts
# (defense-in-depth; TARGET_TABLE is server-side enum-bound before it reaches here).
...
if (!is.null(ols_model)) {
  ...
  if (ncol(X) >= 2) {
    ...
    # VIF flagging is INFORMATIONAL ONLY - nothing is removed from the model.
    # Skip factor/interaction columns (regime dummies, regime:x): their collinearity
    # is STRUCTURAL, not a pathology, and raw VIF is not meaningful for such terms
    # (a Fox-Monette GVIF would be required). Only flag plain named predictors.
    is_structural <- grepl("regime", names(vif_vals)) | grepl(":", names(vif_vals))
    flagged <- names(vif_vals[!is.na(vif_vals) & is.finite(vif_vals) &
                                vif_vals > vif_threshold & !is_structural])
    if (length(flagged) > 0) {
      cat(sprintf("  VIF flagged (informational, > %.1f, NOT removed): %s\n",
                  vif_threshold, paste(flagged, collapse = ", ")))
