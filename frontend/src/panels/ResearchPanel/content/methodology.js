...
export const TESSW = {
  name: 'tessW',
  fullName: 'Contiguity spatial weight matrix for point data based on Voronoi tessellation',
  source: {
...
export const ETA = {
  ...
  paper: {
    ...
    subtitle: 'Interactions of business locations and housing transactions in Warsaw metropolitan area',
    authors: 'Kopczewska, K.',
    year: 2021,
...
ETA <- function(points_sf, region_sf, sample_size) {
  ...
  interpretation: [
    { range: 'ETA > 0.9', meaning: 'Rozkład równomierny (brak aglomeracji)', color: '#10B981' },
    { range: '0.7 < ETA ≤ 0.9', meaning: 'Lekkie skupienie', color: '#F59E0B' },
    { range: '0.5 < ETA ≤ 0.7', meaning: 'Umiarkowane skupienie', color: '#F97316' },
  ...
  warnings: [
    {
      ...
      detail: 'NIE istnieje "eta_local" ani "eta per cell" w oryginalnym spatialWarsaw',
    },
    {
      type: 'info',
  ...
  output: {
    type: 'list',
    properties: [
      { name: 'S_ent', type: 'numeric', description: 'Entropia empiryczna (surowa)' },
      { name: 'H_ent', type: 'numeric', description: 'ETA - główna wartość [0,1]' },
      { name: 'n_points', type: 'integer', description: 'Liczba kafelków/punktów' },
    ],
  },
};
...
export const SAR = {
  ...
  interpretation: [
    { range: 'ρ ≈ 0', meaning: 'Brak autokorelacji przestrzennej', color: '#6B7280' },
    { range: 'ρ > 0', meaning: 'Dodatnia zależność (sąsiedzi podobni)', color: '#10B981' },
    { range: 'ρ < 0', meaning: 'Ujemna zależność (sąsiedzi różni)', color: '#EF4444' },
  ],
  code: `
  ...
  extensions: [
    {
      ...
      implementation: 'forest_cover, distance_to_water, building_density, road_density',
...
export const REMOVED_HALLUCINATIONS = [
  ...
  },
  {
    ...
    `.trim(),
    evidence: 'grep -n "eta_local" /opt/dziki/spatialWarsaw/R/*.R → brak wyników',
    fixed: '2026-01-18',
    fixedIn: 'NEW_01_tessellation.R',
  },
  {
    ...
    `.trim(),
    evidence: 'grep -n "eta_weighted" /opt/dziki/spatialWarsaw/R/*.R → brak wyników',
    fixed: '2026-01-18',
    fixedIn: 'NEW_01_tessellation.R',
  },
  {
    ...
    fixed: '2026-01-18',
    fixedIn: 'NEW_01_tessellation.R zapisuje h_rel_raw = h_rel_rescaled, m_n_factor = 1.0',
  },
];
...
export const PHASE_G_DISCOVERIES = {
  ...
  discoveries: [
    ...
    },
    {
      ...
      `.trim(),
      solution: 'Użyć rozmiar komórki (inverse_area) zamiast count - rozmiary SĄ różne',
    },
    {
      id: 'G3',
      ...
      `.trim(),
      evidence: 'SELECT SUM(CASE WHEN sighting_count=0 THEN 1 ELSE 0 END)... → 76.8%',
    },
    {
      id: 'G4',
  ...
  notUsed: [
    {
      ...
      reason: 'Zakłada continuous Y (ROA firm) - nasze Y to count/point pattern',
    },
    {
      name: 'SPAG()',
...
export const METHODOLOGY_SUMMARY = {
  ...
  spatialWarsawRelation: [
    { element: 'SAR/SEM', status: 'not_used', note: 'Brak wariancji Y (count=1 zawsze)' },
    { element: 'ETA', status: 'inspiration', note: 'Logika: mała komórka = aglomeracja' },
    { element: 'Voronoi (tessW)', status: 'used', note: 'Teselacja zgodna z oryginałem' },
    { element: 'Macierz W', status: 'built_not_used', note: 'Budowana, nie używana w modelu' },
  ],

  ensemble: {
    description: 'Trzy składniki mierzą rozmiar komórki różnymi metodami',
    components: [
      { name: 'density_score', weight: 0.30, method: 'Kernel density estimation' },
      { name: 'spatial_score', weight: 0.40, method: 'Inverse area (główny sygnał)' },
      { name: 'area_rank_score', weight: 0.30, method: 'Area-based rank score (1 - percentile_rank(area))' },
    ],
...
export const COUNT_DATA_PROBLEM = {
  ...
  kopczewskaApproach: {
    ...
    formula: 'roa ~ empl + dummy.prod + dummy.constr + dummy.serv + dist.big.city',
    yVariable: 'ROA (Return on Assets) - continuous, może być ujemna i dodatnia',
  ...
  testResults: {
    ...
    models: [
      { name: 'SEM Gaussian', aic: 330.6, note: 'NAJGORSZE - błędne założenia' },
      { name: 'Poisson GLM', aic: 283.4, note: 'LEPSZE - poprawny rozkład' },
      { name: 'Logistic (0/1)', aic: 237.1, note: 'NAJLEPSZE dla presence/absence' },
    ],
