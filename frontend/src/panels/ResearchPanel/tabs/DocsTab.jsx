...
const Tag = ({ children, color = 'blue' }) => {
  ...
  return (
    <span className={`inline-block px-2 py-0.5 rounded text-xs border font-mono ${colors[color]}`}>
      {children}
    </span>
  );
};
...
const PipelineOverview = () => (
  <div className="space-y-4 text-gray-300 text-sm">
    <h2 className="text-xl font-bold text-white">RESEARCH Mode — Pipeline 8 kroków</h2>
    <p className="text-gray-400">
      Orkiestrator:{' '}
      <code className="font-mono text-green-400">src/analytics/orchestrator_research.py</code>
      {' '}— klasa{' '}
      ...
      <table className="w-full text-xs border-collapse">
        <thead>
          <tr className="border-b border-gray-700">
            <th className="text-left py-2 pr-3 text-gray-500 font-medium">Krok</th>
            <th className="text-left py-2 pr-3 text-gray-500 font-medium">Nazwa</th>
            <th className="text-left py-2 pr-3 text-gray-500 font-medium">Skrypt R</th>
            <th className="text-left py-2 text-gray-500 font-medium">Output</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-800/60">
          {[
            ['01', 'geometry',     '01_generate_voronoi.R',         'tabela w DB (Voronoi lub grid_500)'],
            ['02', 'population',   'research/02_population.R',      'UPDATE population, population_density'],
            ['03', 'osm_features', 'research/03_osm_features.R',    'UPDATE 5 predyktorów OSM'],
            ['04', 'variable_y',   'research/04_variable_y.R',      'UPDATE y_<formula>, spatial_risk'],
            ['05', 'matrix_w',     'research/05_matrix_w.R',        'research_W.rds + w_matrix_edges.geojson'],
            ['06', 'model',        'NEW_02_spatial_models.R',       'research_model.rds'],
            ['07', 'diagnostics',  'research/07_diagnostics.R',     'INSERT → analytics_researchdiagnostics (34 kol.)'],
            ['08', 'results',      '05_ensemble_prediction.R',      'WebSocket pipeline_complete + mapa ryzyka'],
  ...
  ↓ 08_results    — ensemble_risk → WebSocket pipeline_complete → mapa`}</Code>
    ...
    <h3 className="text-lg font-semibold text-white mt-2">Wymiana danych między krokami</h3>
    <p className="text-gray-400">
    ...
    <p className="text-gray-500 mt-2 text-xs">
      Każdy krok tworzy{' '}
      <code className="font-mono">ResearchStepLog</code>{' '}
      z stdout, stderr, exit_code, duration.
      Kody wyjścia: 0=sukces, 1=błąd wejścia, 2=timeout/brak danych, 3=błąd obliczeniowy, 137=OOM.
    </p>
  </div>
);
...
const VariableYNotes = () => (
  <div className="space-y-4 text-gray-300 text-sm">
    ...
    <p className="text-gray-500">
      Skrypt:{' '}
      <code className="font-mono text-green-400">r_scripts/research/04_variable_y.R</code>
    </p>

    <h3 className="text-lg font-semibold text-white mt-4">Dlaczego RESEARCH {'≠'} PUB</h3>
    <p>
      W trybie PUB (Voronoi 1:1) każda komórka ma dokładnie 1 obserwację —
      {' '}<code className="font-mono text-gray-300">count = 1</code> zawsze → brak wariancji w Y → SAR/SEM niemożliwy.
    </p>
    <p className="mt-2">
    ...
    <h3 className="text-lg font-semibold text-white mt-4">Walidacja konfiguracji</h3>
    <p className="text-gray-400">
      <code className="font-mono text-green-400">ResearchConfig</code> odrzuca kombinacje
      Voronoi + BINARY — znowu problem count=1 per komórka Voronoi (brak wariancji w Y binarnym).
      Źródło:{' '}
      <code className="font-mono text-gray-300">src/analytics/models_research.py</code>.
    </p>
  </div>
);
...
const MatrixWNotes = () => (
  <div className="space-y-4 text-gray-300 text-sm">
    <h2 className="text-xl font-bold text-white">Krok 5: Macierz wag przestrzennych W</h2>
    <p className="text-gray-500">
      Skrypt:{' '}
      <code className="font-mono text-green-400">r_scripts/research/05_matrix_w.R</code>
    </p>
    <p>
      Macierz W definiuje "sąsiedztwo" — które komórki wpływają na które w modelu SAR/SEM.
      Output:{' '}
      <code className="font-mono text-green-400">/app/data/research_W.rds</code>
      {' '}(obiekt{' '}
      <code className="font-mono text-gray-300">listw</code> z pakietu spdep).
    </p>

    <h3 className="text-lg font-semibold text-white mt-4">Trzy metody (RESEARCH_W_METHOD)</h3>
    <div className="space-y-4">
      ...
      <div className="border-l-4 border-green-500 pl-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-gray-500 text-xs">k-nearest neighbours z AIC</span>
        </div>
        <p className="text-gray-400">
          Iteruje k od{' '}
          ...
          {' '}wg metodologii Kopczewskiej — własna impl. (05_matrix_w.R:140–200).
        </p>
      </div>
      ...
      <div className="border-l-4 border-purple-500 pl-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-gray-500 text-xs">wagi proporcjonalne do wspólnej granicy</span>
        </div>
        <p className="text-gray-400">
          Queen contiguity z wagami proporcjonalnymi do długości wspólnej krawędzi.
          Bezpośrednio inspirowany funkcją{' '}
          <code className="font-mono text-gray-300">tessW()</code>
          {' '}wg metodologii Kopczewskiej — własna impl. (05_matrix_w.R:283–314).
        </p>
      </div>
    </div>

    <h3 className="text-lg font-semibold text-white mt-4">Wzory (row-standardized)</h3>
    ...
    <p className="text-gray-400">
      <code className="font-mono text-gray-300">poly2nb()</code> wymaga układu metrycznego.
      Kolumna{' '}
      <code className="font-mono text-green-400">geometry_2180</code>
      {' '}(EPSG:2180) zawsze generowana w{' '}
      <code className="font-mono text-gray-300">GridCell</code>
      {' '}(migration 0003) — brak przeliczenia CRS w skrypcie R.
    </p>
  </div>
);
...
const ModelNotes = () => (
  <div className="space-y-4 text-gray-300 text-sm">
    <h2 className="text-xl font-bold text-white">Krok 6: Model przestrzenny SAR/SEM/SDM</h2>
    <p className="text-gray-500">
      Skrypt:{' '}
      <code className="font-mono text-green-400">r_scripts/NEW_02_spatial_models.R</code>
    </p>
    <p className="text-gray-500 text-xs">
      Wspólny skrypt PUB i RESEARCH. W RESEARCH czyta{' '}
      <code className="font-mono">research_W.rds</code> z kroku 5,
      zapisuje{' '}
      <code className="font-mono">research_model.rds</code>.
      Diagnostyki dopiero w kroku 7.
    </p>

    <h3 className="text-lg font-semibold text-white mt-4">Auto-selekcja (RESEARCH_MODEL_TYPE=auto)</h3>
    <p className="text-gray-400">
      Krok 6 fituje SAR i SEM przez MLE ({' '}
      <code className="font-mono">lagsarlm</code>,{' '}
      <code className="font-mono">errorsarlm</code>{' '}
      z pakietu spatialreg). Auto-selekcja przez porównanie AIC:{' '}
    ...
    <p className="text-gray-400 mt-2 text-xs">
      ...
      <code className="font-mono">07_diagnostics.R</code>
      ) jako post-hoc diagnostics — nie kryterium wyboru modelu.
    </p>
    ...
    <div className="space-y-3">
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-gray-500 text-xs">Spatial Autoregressive / Lag Model</span>
        </div>
        <Formula>y = ρWy + Xβ + ε</Formula>
        <p className="text-gray-400 text-xs">
          ρ (rho) — autokorelacja przestrzenna odpowiedzi. Sąsiedzi wpływają na wartość Y.
        </p>
      </div>
      <div>
        <div className="flex items-center gap-2 mb-1">
        ...
        <p className="text-gray-400 text-xs">
          λ (lambda) — autokorelacja przestrzenna błędów. Pominięte zmienne przestrzenne.
        </p>
      </div>
      <div>
        <div className="flex items-center gap-2 mb-1">
    ...
    <p className="text-gray-400">
      Zmienne niezależne X (z kroku 3, standaryzowane z-score):
      forest_cover, road_density, building_density, distance_to_water, barrier_resistance.
      Aktywny zestaw konfigurowalny przez{' '}
      <code className="font-mono text-gray-300">RESEARCH_ACTIVE_PREDICTORS</code>.
    </p>
...
const DiagnosticsNotes = () => (
  <div className="space-y-4 text-gray-300 text-sm">
    ...
    <p className="text-gray-500">
      Skrypt:{' '}
      <code className="font-mono text-green-400">r_scripts/research/07_diagnostics.R</code>
    </p>
    <p className="text-gray-400">
      Jedyne miejsce zapisu do tabeli{' '}
      <code className="font-mono text-green-400">analytics_researchdiagnostics</code>
      {' '}(34 kolumny). Odczytuje{' '}
      <code className="font-mono text-gray-300">research_W.rds</code>
      {' '}i{' '}
      <code className="font-mono text-gray-300">research_model.rds</code>
      {' '}z kroków 5 i 6.
    </p>
    ...
    <div className="space-y-5 mt-4">
      ...
      <div className="border-l-4 border-yellow-500 pl-4">
        ...
        <p className="text-gray-400 mt-1">
          Lagrange Multiplier tests z pakietu spdep. Podstawa auto-selekcji SAR vs SEM.
        </p>
        <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-xs font-mono">
      ...
      <div className="border-l-4 border-purple-500 pl-4">
        ...
        <p className="text-gray-400 mt-1">
          Local Indicators of Spatial Association (Local Moran I).
          Klasyfikuje komórki do klastrów przestrzennych.
        </p>
        <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-xs font-mono">
          <span className="text-green-400">lisa_hh_count</span>
          <span className="text-gray-400">High-High — skupisko wysokiego ryzyka</span>
          <span className="text-green-400">lisa_ll_count</span>
          <span className="text-gray-400">Low-Low — skupisko niskiego ryzyka</span>
      ...
      <div className="border-l-4 border-green-500 pl-4">
        ...
        <p className="text-gray-400 text-xs">0 = silna aglomeracja, 1 = rozkład równomierny</p>
        <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-xs font-mono">
      ...
      <div className="border-l-4 border-gray-600 pl-4">
        ...
        <p className="text-gray-400 mt-1">
          ...
          (<code className="font-mono text-gray-300">RESEARCH_VIF_THRESHOLD</code>, domyślnie 5.0).
          Predyktory powyżej progu odrzucane
          (<code className="font-mono text-green-400">predictors_dropped</code>).
        </p>
        <p className="text-gray-400 mt-2">
          Spatial impacts dla SAR/SDM: direct/indirect/total effects
          wg LeSage {'&'} Pace (2009). Dostępne przez{' '}
          <code className="font-mono text-green-400">/api/research/impacts/</code>.
        </p>
      </div>
    </div>
  </div>
