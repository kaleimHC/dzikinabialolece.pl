...
const PARAM_HINTS = {
  ...
  // Diagnostyka
  vif_threshold:
    "Próg multikolinearności (Variance Inflation Factor). VIF jest informacyjny - zmienne powyżej progu są tylko oznaczane, nie usuwane. 5 = restrykcyjny, 10 = tolerancyjny.",
  alpha:
  ...
  run_lm_tests:
    "Testy LM (Lagrange Multiplier) - post-hoc diagnostyka. Selekcja SAR vs SEM idzie po AIC, nie po LM.",
  run_lisa: "LISA - lokalne wskaźniki autokorelacji (hot-spots, cold-spots).",
  run_eta: "ETA - relatywna entropia tessellacji (własna implementacja).",
};
...
const Y_FORMULA_CHOICES = [
  {
    value: "log_intensity",
    label: "log_intensity (intensywność zgłoszeń / pole) - default",
    desc: "Y = log((obserwacje + 1) / area_km2). Korekta +1 utrzymuje puste komórki skończone. Domyślna zmienna zależna.",
  },
  {
    value: "log_count",
    label: "log(obserwacje+1)",
    desc: "Bezpośrednia miara dzików (na siatce ~stałej powierzchni zbliżona do log_intensity)",
  },
  {
    value: "inv_pop",
    label: "1 / populacja (proxy)",
    desc: "Proxy opt-in: gęstość zaludnienia (odwrotna)",
  },
  {
    value: "log_pop",
    label: "log(populacja) (proxy)",
    desc: "Proxy opt-in: normalizacja rozkładu populacji",
  },
  {
    value: "count_pop",
...
const W_METHOD_CHOICES = [
  {
    value: "contiguity",
    label: "Queen contiguity - principled (nieuruchamiana)",
    desc: "Sąsiedztwo Queen (poly2nb): wspólna krawędź lub wierzchołek. Zasadnicza metoda W, ale NIE operacyjny default (nieuruchamiana).",
  },
  {
    value: "knn_aic",
    label: "KNN (auto k) - operacyjny default",
    desc: "Operacyjny default: k-NN z doborem k przez AIC na formie lag (k_min=2). Heurystyka wrażliwości, nie selektor zasadniczy.",
  },
  {
    value: "tessw",
    label: "tessW (spatialWarsaw)",
    desc: "Contiguity z wagami długości wspólnej granicy",
  },
];
...
const REGIME_TYPE_CHOICES = [
  {
    value: "none",
    label: "Pooled (wyłączony) - default",
    desc: "Domyślnie: jeden model globalny (pooled), bez podziału na reżimy",
  },
  {
    value: "trinary",
    label: "Trinary (las/miasto/mixed) - opt-in, regime switching",
    desc: "Opt-in: osobny intercept per reżim (0 + regime). Regime interaguje tylko z predyktorami nie-definiującymi; forests/buildings zostają jako pooled (unika self-collinearity)",
  },
];
...
const EMPTY_CONFIG = {
  name: "",
  geometry_type: "grid_500", // research geometry (regular 500 m grid)
  population_method: "spatial_join",
  y_formula: "log_intensity", // default: Y = log((count + 1) / area_km2)
  w_method: "knn_aic", // operacyjny default (contiguity/tessW principled, ale nieuruchamiane)
  k_range_min: 2,
  k_range_max: 30, // only used when w_method = knn_aic
  model_type: "auto", // SAR vs SEM by AIC
  active_predictors: [],
  run_moran: true,
  run_lm_tests: true,
  run_lisa: false,
  ...
  alpha: 0.05,
  seed: 42,
  // Regime model - opt-in (default pooled; regime interacts only with non-defining predictors)
  use_regime_model: false,
  regime_type: "none",
  regime_threshold: 0.3, // forest threshold (used only when regime enabled)
  regime_threshold_urban: 0.15, // urban threshold (used only when regime enabled)
};
