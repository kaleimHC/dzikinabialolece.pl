...
3. MONTE CARLO CSR TEST (nowy 07b_entropy_test.R lub w 07_diagnostics.R):
  ...
  },
  {
    ...
    adopted:
      'Punkt odniesienia metodologiczny dla koncepcji dryfu przestrzennego (spatial drift) w predykcji spotkań z dziką zwierzyną. Główna teza artykułu (komunikacja miejska w Dreźnie, N=391 dzielnic, 2005): relacje między zmiennymi a wynikiem rzadko są stałe w przestrzeni, a ignorowanie tej niestacjonarności prowadzi do uśredniania efektów (averaging effects). Dosłowny cytat: "On a global scale (neglecting these spatial differences) the effect of the promotion might be averaged out."\n\nUżycie w naszym projekcie: zachowanie dzików w Białołęce może wykazywać regionalne odwrócenie (regional inversion) — bliskość człowieka przyciąga w jednym klastrze, odpycha w innym. Globalny model SAR/SEM może wskazać wynik bliski zeru, jeśli te efekty się zniosą. Liczebność próby wystarczająca według punktu odniesienia Müllera — nasze 3500 zweryfikowanych zgłoszeń (lub PUB 1:1 z próbą, 3500 przy pełnej) > N=391 Müllera.\n\nMoja interpretacja: wykrywanie dryfu wymaga przepływu GWR + klastrowania współczynników (zgodnie z metodologią Müllera). Świadoma decyzja: poza aktualnym zakresem portfolio — etap 6 publikacji koncentruje się na globalnym SAR/SEM z diagnostyką post-hoc LISA. Pełna analiza dryfu = praca po publikacji, wymaga reaktywacji GWR (aktualnie zaszłość techniczna).',
    rejected:
      "Bezpośredniej adopcji przepływu Müllera: GWR + testy F1/F3 Leunga + k-means klastrowania współczynników. Wymagałaby reaktywacji przepływu GWR, który świadomie oznaczono jako zaszłość techniczną (commit 88b4049). Aktualnie stosujemy podejście wyłącznie SAR/SEM z post-hoc wykrywaniem klastrów LISA HH/LL/HL/LH (research/07_diagnostics.R:299-327) — częściowe wychwytywanie sygnału dryfu bez pełnej regresji lokalnej. Kompromis: stabilność + prostsza architektura kontra utrata wykrywania lokalnej niestacjonarności.",
    inCode:
...
const LECTURES = [
  {
    ...
    doi: null,
    adopted:
      "moran.test() dla globalnej Morana I (krok 7), localmoran() → LISA quadranty, struktura workflow: Moran I OLS residuów → sygnał autokorelacji → decyzja o modelu przestrzennym.\n\nMoja interpretacja: ta struktura workflow stała się szkieletem kroku 7 przepływu — OLS → Moran I residuów → jeśli istotny → SAR/SEM. Moran I nie jako finalna diagnostyka, ale jako kryterium selekcji: czy w ogóle model przestrzenny jest potrzebny.",
    rejected:
  ...
  },
  {
    ...
    doi: null,
    adopted:
      'Konceptualne rozróżnienie: autokorelacja przestrzenna (podobieństwo sąsiadów) vs. dryf przestrzenny (kierunkowość efektów). Oba procesy mogą współistnieć — SAR/SEM absorbuje autokorelację, ale dryf wymaga diagnostyki reszt.\n\nFormalne równanie GWR: yᵢ = β₀(uᵢ,vᵢ) + Σₖ xᵢ,ₖ βₖ(uᵢ,vᵢ) + εᵢ, gdzie (uᵢ,vᵢ) są współrzędnymi geograficznymi, a βₖ(uᵢ,vᵢ) to lokalne współczynniki estymowane per obserwacja z ważoną metodą najmniejszych kwadratów (Brunsdon et al. 1996). Funkcje jądra: Gaussian (łagodny zanik), Bi-square (zero poza przepustowością, bandwidth), Adaptive vs Fixed. Selekcja przepustowości: AICc preferowane nad CV dla danych ekologicznych ("more robust than CV for small N").\n\nProgi stabilności per Zajęcia 6: "N < 100: Do NOT use GWR — exploratory only. Unacceptable non-convergence; inflated Type I errors". "100 < N < 200: GWR wciąż wysoce niestabilny". "N = 500-5,000 ACCEPTABLE Stable convergence". Nasz tryb FAST (9875 cells) > akceptowalny górny próg, tryb PUB (1:1 z próbą, 3500 przy pełnej) w akceptowalnym zakresie, ale tryby FAST/PUB wybrane ze względu na szybkość i spójność metodologiczną, nie pod kątem GWR.',
    rejected:
      "Aktywny przepływ GWR. Świadoma migracja GWR → SAR/SEM w refaktoryzacji 2026 z trzech powodów: (1) audytowalność — jeden globalny model z interpretowalnym ρ/λ łatwiejszy do weryfikacji niż 3500+ lokalnych modeli z różnymi współczynnikami; (2) stabilność — SAR/SEM stabilny dla całego naszego zakresu (PUB 1:1 z próbą, FAST 9875 cells); (3) prostsza architektura — jeden model SAR/SEM + wykrywanie klastrów LISA post-hoc lepiej pasuje do zakresu portfolio niż pełny GWR + analiza współczynników per kafel.\n\nZaszłości: test_r_pipeline.py oznaczony @pytest.mark.skip (commit 88b4049) odwołuje się do starych tabel analytics_gwr_result nieistniejących w aktualnym schemacie DB. Pakiet GWmodel usunięty z Dockerfile.r (commit 61f983b).",
    inCode:
      "r_scripts/research/07_diagnostics.R:299-327 (moran.test na resztach modelu — test dryfu przestrzennego po estymacji SAR/SEM)",
  },
  {
  ...
  },
  {
    ...
    rejected:
      "Pełny przepływ BootSpatReg() — wymaga próby min. 50K obserwacji do sensownego resamplingu podpróby. Nasz tryb PUB (1:1 z próbą, 3500 przy pełnej) lub tryb FAST (9875 cells) — bootstrap subsampling teoretycznie możliwy, ale wymaga min. 50K obserwacji dla sensownego resamplingu per BootSpatReg() spec. Poza aktualnym zakresem portfolio.",
    inCode:
      "r_scripts/05_ensemble_prediction.R:146-154 (wagi ensemble: W_DENSITY=0.30, W_SPATIAL=0.40, W_ETA=0.30)",
  },
  {
    ...
    doi: null,
    adopted:
      'Punkt odniesienia metodologiczny dla analizy stabilności klastrów w czasie i przestrzeni. Algorytm STS (Kopczewska 2021, Land Use Policy): (1) estymacja GWR per okres, (2) k-means klastrowania współczynników per okres, (3) rasteryzacja + mediana ID klastra per kafel siatki, (4) porównanie podziałów między okresami z Adjusted Rand Index (ARI), (5) wyjście: macierz kolorów okres-do-okresu.\n\nFormalna definicja problemu: "The spatio-temporal stability problem is to detect if the similarity of coefficients, indicating whether they should belong to the same cluster, is stable in time over space".\n\nMoja interpretacja: STS jest konceptem trafnym dla monitoringu fauny (wzorce sezonowe + 5-letni zakres danych), ale blokery techniczne: (a) funkcja STS() w spatialModel wymaga T≥2 okresów ("Wymagane: dane z kilku okresów (T ≥ 2, inaczej funkcja zwróci błąd)"); (b) podział 3500 zgłoszeń na lata daje per-period N ~700 (powyżej progu katastrofy N<500) — stabilność GWR nie jest już blokerem, ale GWR to aktualnie zaszłość techniczna (zob. wpis Zajęcia 6: N<500 catastrophic); (c) GWR to aktualnie zaszłość techniczna w naszym przepływie.',
    rejected:
      "Pełnej analizy stabilności przestrzenno-czasowej w aktualnym przepływie. Wymagane warunki wstępne NIE spełnione: (a) reaktywacja GWR (aktualnie zaszłość, zob. wpis Zajęcia 6 GWR), (b) wystarczające N per okres, (c) metodologia stratyfikacji sezonowej, (d) zebranie rozszerzonej serii czasowej. Ważne rozróżnienie względem BootSpatReg() (często mylone): BootSpatReg = przestrzenna regresja bootstrapowana z selekcją medoidów PAM dla dużych danych; STS = stabilność klastrów oparta na ARI między okresami. Świadoma decyzja: STS odroczone po publikacji jako rozszerzenie wielookresowe.",
    inCode:
