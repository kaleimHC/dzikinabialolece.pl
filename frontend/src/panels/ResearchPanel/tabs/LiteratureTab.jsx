...
const PAPERS = [
  {
    ...
    rejected:
      "Surowy ETA jako zmienna Y modelu — dziki nie tworzą skupisk jak firmy, ETA mierzy skupienie wzorca punktowego, nie ciągłe ryzyko na obszarze. Y musi być continuous per kafel (count_pop / inv_pop / log_count), nie globalny skalar entropii porównujący ze stanem maksymalnego rozproszenia.\n\nBivariate cross-product entropy między dwoma typami punktów — paper analizuje business + population OSOBNO (univariate) i porównuje wizualnie, brak łącznej formuły matematycznej. To research gap, nie świadome odrzucenie.\n\nShannon Redundancy O = 1 − relH (Eq 5 paper) — mathematically equivalent do 1 − H_rel, czyli redundantne raportowanie.",
    inCode:
      "r_scripts/research/07_diagnostics.R:465-516 (H_emp, H_max, H_rel — własna impl. Shannona). Tessellation setup w 02_voronoi.R (paper proponuje Voronoi tessellation jako foundation methodology).",
    todoImpl: {
      priority: "HIGH",
      summary:
...
const LECTURES = [
  ...
  },
  {
    ...
    doi: null,
    adopted:
      'Z wykładu zaadoptowałem framework efektów spillover: aktywność dzicza w kaflu i wpływa na sąsiadów j przez człon ρWy w SAR. Tobler\'s First Law jako formalne uzasadnienie że bliskość przestrzenna ma znaczenie — bez tego założenia kNN i tessW byłyby arbitralne.\n\nMoja interpretacja: "spatial lag transmituje ryzyko" to nie metafora — to formalny mechanizm ρWy gdzie W = macierz wag (tessW lub kNN). Im wyższy ρ, tym silniejszy spillover. Dla dzika: ryzyko wylewa się z hotspotów do sąsiednich kafli, co SAR modeluje explicite.',
    rejected:
  ...
  },
  {
    ...
    doi: null,
    adopted:
      'Konceptualne rozróżnienie autokorelacja przestrzenna (podobieństwo sąsiadów) vs. dryf przestrzenny (kierunkowość efektów). Rozumienie że oba procesy mogą współistnieć – SAR/SEM absorbuje autokorelację, ale dryf wymaga diagnostyki residuów.\n\nFormal GWR equation: yᵢ = β₀(uᵢ,vᵢ) + Σₖ xᵢ,ₖ βₖ(uᵢ,vᵢ) + εᵢ, gdzie (uᵢ,vᵢ) są współrzędnymi geograficznymi a βₖ(uᵢ,vᵢ) lokalne współczynniki estymowane per obserwacja z weighted least squares (Brunsdon et al. 1996). Kernel functions: Gaussian (smooth decay), Bi-square (zero outside bandwidth), Adaptive vs Fixed. Bandwidth selection: AICc preferred over CV dla ecological data ("more robust than CV for small N").\n\nStability thresholds per Zajęcia 6: "N < 100: Do NOT use GWR — exploratory only. Unacceptable non-convergence; inflated Type I errors". "100 < N < 200: GWR wciąż wysoce niestabilny". "N = 500-5,000 ACCEPTABLE Stable convergence". Nasz FAST mode (9875 cells) > acceptable upper, PUB mode (1:1 z próbą, 3500 przy pelna) w acceptable range, ale FAST/PUB mode wybrane jest dla speed/methodology consistency, nie GWR-friendliness.',
    rejected:
      "Aktywne GWR pipeline. Świadoma migracja GWR → SAR/SEM w refactor 2026 z trzech powodów: (1) audytowalność — single global model z interpretable ρ/λ łatwiejsze do verify niż 3500+ lokalnych modeli z różnymi współczynnikami; (2) stability — SAR/SEM stabilny dla całego naszego range (PUB 1:1 z próbą, FAST 9875 cells); (3) simpler architecture — SAR/SEM jeden model + post-hoc LISA cluster detection lepiej fits portfolio scope niż full GWR + per-cell coefficient analysis.\n\nLegacy artifacts: test_r_pipeline.py marked @pytest.mark.skip (commit 88b4049) references stare tabele analytics_gwr_result które nie istnieją w current DB schema. GWmodel package removed z Dockerfile.r (commit 61f983b).",
    inCode:
      "r_scripts/research/07_diagnostics.R:299-327 (moran.test na resztach modelu — test dryfu przestrzennego po estymacji SAR/SEM)",
  },
  {
  ...
  },
  {
    ...
    doi: null,
    adopted:
      'Idea PAM clustering do wyboru reprezentatywnego podzbioru danych (inspiracja dla logiki ensemble). Voronoi jako kalibracja przestrzeni przy predykcji: nowe punkty "wpadają" do istniejących kafli przez tessellację.',
    rejected:
      "Pełen BootSpatReg() pipeline – wymaga próby min. 50K obserwacji do sensownego resampligu subpróby. Nasz PUB mode (1:1 z próbą, 3500 przy pelna) lub FAST mode (9875 cells) — bootstrap subsampling teoretycznie możliwy ale wymaga min. 50K obserwacji dla sensownego resamplingu per BootSpatReg() spec. Out of current portfolio scope.",
    inCode:
      "r_scripts/05_ensemble_prediction.R:146-154 (wagi ensemble: W_DENSITY=0.30, W_SPATIAL=0.40, W_ETA=0.30)",
  },
  {
