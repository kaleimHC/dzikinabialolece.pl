...
const fetchComputedResearchGeometry = async () => {
  try {
    ...
    const lastSuccess = status?.last_run?.status === 'success' ? status.last_run : null;
    if (lastSuccess) {
      const successConfig = (configsData.configs || []).find(c => c.id === lastSuccess.config_id);
...
export default function RecalcPanel({ embedded = false }) {
  const { fetchSightings, displayMode, setDisplayMode, setResearchGeometry } = useSightingsStore();
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const justCompleted = isCompleted && !isRunning && computeStatus.running;
        ...
        if (justCompleted) {
          fetchSightings();
          setTimeout(() => window.dispatchEvent(new CustomEvent('voronoi-refresh')), 500);
          setTimeout(() => setComputeStatus(s => ({ ...s, completed: false, taskId: null })), 3000);
        }
      } catch (e) {
    ...
    try {
      if (isFastMode) {
        ...
        if (res.ok) {
          ...
          fetchSightings();
          setTimeout(() => window.dispatchEvent(new CustomEvent('voronoi-refresh')), 500);
          setTimeout(() => setComputeStatus(s => ({ ...s, completed: false })), 3000);
        } else {
        ...
        if (res.ok && data.task_id) {
          ...
          throw new Error(data.message || `${pipelineMode} pipeline failed to start`);
        }
      }
    } catch (e) {
      setComputeStatus(prev => ({
  ...
  const progress = computeStatus.totalSteps > 0
  ...
  const panelContent = (
    <>
          ...
          return (
            <button
              ...
              className={`flex-1 px-3 py-2 rounded-md text-xs font-medium transition-all ${
                displayMode !== mode.id ? 'bg-gray-700 text-gray-400 hover:bg-gray-600' : ''
              }`}
              style={displayMode === mode.id ? getModeStyle(mode.id, tk) : undefined}
            >
              {mode.label}
            </button>
          );
        ...
        <button
          ...
          style={{
            ...
            background: computeStatus.running
              ...
              : isFastMode ? tk.ryjowisko : tk.encounter,
            color: computeStatus.running ? '#fff' : isFastMode ? contrastColor(tk.ryjowisko) : contrastColor(tk.encounter),
            display: 'flex',
            alignItems: 'center',
      ...
      {computeStatus.completed && !computeStatus.running && (
        <div style={{ marginTop: '8px', fontSize: '10px', color: 'rgb(var(--color-primary))' }}>
          Gotowe!
        </div>
      )}
