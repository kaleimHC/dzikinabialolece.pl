...
export default function App() {
  const { fetchSightings, showResearchPanel, isAddMode, enterAddMode, displayMode, showFastGrid, showHeatmap, toggleFastGrid, toggleHeatmap, hasSeenOnboarding } = useSightingsStore();
  ...
  useEffect(() => {
    fetchSightings();
    loadFromStorage();
    const handleOnline = () => { processQueue(); fetchSightings(); };
            ...
            <AnimatePresence>
              {!anyBarOpen && (
                <motion.div
                  ...
                  style={{ bottom: 'calc(1rem + env(safe-area-inset-bottom, 0px))', gridTemplateColumns: '1fr 56px 1fr' }}
                >
                  {/* Próba pill */}
                  <motion.button
